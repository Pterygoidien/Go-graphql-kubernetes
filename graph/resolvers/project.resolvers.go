...
func (r *projectMutationResolver) AddImage(ctx context.Context, obj *model.ProjectMutation, newImage model.NewImageInput) (bool, error) {
	...
	if newImage.Description != nil {
		description = sql.NullString{String: *newImage.Description, Valid: true}
	}
...
func (r *projectMutationResolver) UpdateImageDescription(ctx context.Context, obj *model.ProjectMutation, id string, newDescription *string) (bool, error) {
	err := r.queries.UpdateImageDescription(context.Background(), sqlc.UpdateImageDescriptionParams{ID: uuid.MustParse(id), Description: sql.NullString{
...
func (r *projectMutationResolver) UpdateLocation(ctx context.Context, obj *model.ProjectMutation, newLocation model.LocationInput) (bool, error) {
	err := r.queries.SetProjectLocation(context.Background(), sqlc.SetProjectLocationParams{
		ID: uuid.MustParse(obj.ID),
		Location: sql.NullString{
			String: newLocation.Name,
			Valid:  newLocation.Name != "",
		},
	})
	return err == nil, err
}

func (r *projectMutationResolver) SetVisibility(ctx context.Context, obj *model.ProjectMutation, visibility bool) (bool, error) {
	err := r.queries.SetProjectVisibility(context.Background(), sqlc.SetProjectVisibilityParams{
		ID:     uuid.MustParse(obj.ID),
		Public: visibility,
	})
	return err == nil, err
...
func (r *queryResolver) SearchProjects(ctx context.Context, searchString string, options model.SearchOptions, offset int, countLimit int) ([]*model.Project, error) {
	...
	var dbResults []sqlc.ProjectsRanked
	var err error
	if options.Tag != nil {
	...
	results := model.ProjectsFromRankedDBProjects(dbResults)
	return results, nil
}
